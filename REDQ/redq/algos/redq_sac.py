...
def train(self, logger):
    ...
    for i_update in range(num_update):
        ...
        # Backpropagate Q loss and update Q networks
        for q_i in range(self.num_Q):
        ...
        # Update policy and alpha (entropy weight) every `policy_update_delay` steps
        if ((i_update + 1) % self.policy_update_delay == 0) or i_update == num_update - 1:
            ...
            # Backpropagate policy loss and update policy network
            self.policy_optimizer.zero_grad()
        policy_loss.backward()
        self.policy_optimizer.step()

        # Calculate alpha loss and update alpha if using automatic alpha adjustment
        if self.auto_alpha:
